...
@Component({
  selector: 'app-modal-dettalgi-gestione-contenziosi',
  templateUrl: './modal-dettalgi-gestione-contenziosi.component.html',
  styleUrls: ['./modal-dettalgi-gestione-contenziosi.component.scss']
})
export class ModalDettalgiGestioneContenziosiComponent implements OnInit {
  ...
  private stepperService: StepperService, private dialogRef: MatDialogRef<ModalDettalgiGestioneContenziosiComponent>,
  ...
  loadData(){
    ...
    // this.ruolo = this.storageService.getItem('role');
    ...
    this.rilevazioniService.getDettagliSegnalazioneContenziosoById(this.idSegnalazione).subscribe({
      next:(aspettiSegnalazioneContezioso:AspettoProdotti[])=>{
        this.datiAspetto = aspettiSegnalazioneContezioso;
        ...
        this.documentiService.findAllegatiAspetti(this.idSegnalazione, this.idAspetti, "Contenzioso", "").subscribe({
        ...
        this.documentiService.findAllegatoSegnalazione(this.idSegnalazione, '', "Contenzioso").subscribe({
          next: (res) =>{
        ...
        this.documentiService.findAllegatoSegnalazione(this.idSegnalazione, ruolo, "Contenzioso").subscribe({
          next: (res) => {
  ...
  risolviSegnalazione(idStatoSalvataggio: number){
    ...
    this.rilevazioniService.updateIntegrazioniContezioso(integrazioni, this.storageService.getItem('matricola'), idStatoSalvataggio).subscribe({
      next:() => {
  ...
  handleRowClick(idAspettoSegnalazione: number){
    ...
    const dialogRef = this.dialog.open(ModalRisolviContenziosoComponent, dialogConfig);
    dialogRef.afterClosed().subscribe(()=>{
      const dialogRef2 = this.dialog.open(ModalDettalgiGestioneContenziosiComponent,{
        width: 'auto',
        height: 'auto',
        autoFocus: true,
  ...
  downloadFileAspetto(idAspetto: number){
    this.documentiService.getFileAspetti(this.idSegnalazione, idAspetto, "Contenzioso", "").subscribe(response => {
  ...
  downloadFileSegnalazione(){
    this.documentiService.getFileSegnalazione(this.idSegnalazione, "Contenzioso", "").subscribe(response =>{
      const contentDisposition = response.headers.get('content-disposition');
      const fileName = this.getFileNameFromContentDisposition(contentDisposition);
  ...
  toggleAllegati() {
    ...
    if(!this.isFile || this.showGestioneFile)
  ...
  removeFileSegnalazione(file: any) {
    if (
      ...
      this.isFile = false;  // Disabilita il flag
    }
  }
  ...
  allegaFile() {
    ...
    const ruolo = this.selectRole();
    this.documentiService.allegaFileSegnalazione(this.idSegnalazione, ruolo, "Contenzioso", this.fileSegnalazione).subscribe({
